...
function ymiatPathDepth() {
  ...
  return hasFile ? parts.length - 1 : parts.length;
}

function ymiatRelativePrefix(depth) {
  if (depth <= 0) return '';
  return '../'.repeat(depth);
}

function ymiatIsLocalHost() {
  const protocol = window.location.protocol;
  const hostname = window.location.hostname;
  return protocol === 'file:' || hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '';
}

/** Locale content root (EN site root or .../nl/). */
function getRootPath() {
  const locale = ymiatDetectLocale();
  const depth = ymiatPathDepth();
  if (ymiatIsLocalHost()) {
    let contentDepth = depth;
    if (locale === 'nl') {
      contentDepth = Math.max(0, depth - 1);
    }
    return ymiatRelativePrefix(contentDepth);
  }
  const base = BASE_PATH ? (BASE_PATH.endsWith('/') ? BASE_PATH : BASE_PATH + '/') : '/';
  return locale === 'nl' ? base + 'nl/' : base;
}

/** Shared assets root (never under /nl/). */
function getAssetsPath() {
  const depth = ymiatPathDepth();
  if (ymiatIsLocalHost()) {
    return ymiatRelativePrefix(depth);
  }
  if (BASE_PATH) {
...
function ymiatChrome(locale) {
  const pack = (window.YMIAT_I18N_CHROME && window.YMIAT_I18N_CHROME[locale])
    || (window.YMIAT_I18N_CHROME && window.YMIAT_I18N_CHROME.en)
    || null;
  return pack;
}

function ymiatEscapeHtml(s) {
  return String(s)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function ymiatLocalizeCharactersNav(template, n) {
  let html = template;
  const reps = [
    ['>Characters</a>', '>' + n.characters + '</a>'],
    ['>Character Creation</a>', '>' + n.characterCreation + '</a>'],
    ['>Character Creator</a>', '>' + n.characterCreator + '</a>'],
    ['>Classes</span>', '>' + n.classes + '</span>'],
    ['>All classes</a>', '>' + n.allClasses + '</a>'],
    ['>Class abilities index</a>', '>' + n.classAbilitiesIndex + '</a>'],
    ['>Lineages</span>', '>' + n.lineages + '</span>'],
    ['>All lineages</a>', '>' + n.allLineages + '</a>'],
    ['>Heritages</span>', '>' + n.heritages + '</span>'],
    ['>All heritages</a>', '>' + n.allHeritages + '</a>'],
    ['>Backgrounds</span>', '>' + n.backgrounds + '</span>'],
    ['>All backgrounds</a>', '>' + n.allBackgrounds + '</a>'],
    ['>Talents</span>', '>' + n.talents + '</span>'],
    ['>All talents</a>', '>' + n.allTalents + '</a>'],
    ['class="nav-submenu-link">Magic</a>', 'class="nav-submenu-link">' + n.talentMagic + '</a>'],
    ['class="nav-submenu-link">Martial</a>', 'class="nav-submenu-link">' + n.talentMartial + '</a>'],
    ['class="nav-submenu-link">Utility</a>', 'class="nav-submenu-link">' + n.talentUtility + '</a>'],
  ];
  for (let i = 0; i < reps.length; i++) {
    html = html.replace(reps[i][0], reps[i][1]);
  }
  return html;
}

function ymiatBuildNav(locale) {
  ...
  return (
    ...
    '<a href="{{ROOT}}rules/core.html#dice-and-tests">' + e(n.diceAndTests) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#ability-scores">' + e(n.abilitiesAndProperties) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#determining-ability-modifiers">' + e(n.determiningAbilityModifiers) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#proficiency-and-advantage">' + e(n.proficiencyBonus) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#skills-and-ability-checks">' + e(n.skillsAndAbilityChecks) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#talents">' + e(n.talents) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#wounds-and-scale">' + e(n.woundsAndScale) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#level-advancement">' + e(n.levelAdvancement) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#attacks-and-defense">' + e(n.attacksAndDefense) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#weapon-attacks">' + e(n.weaponAttacks) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#weapon-mastery">' + e(n.weaponMastery) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#actions-and-time-encounter-mode">' + e(n.actionsAndTime) + '</a>' +
    '<a href="{{ROOT}}rules/core.html#magic-and-spell-resources">' + e(n.magicAndSpellResources) + '</a>' +
...
function ymiatBuildFooter(locale) {
  const chrome = ymiatChrome(locale);
  const f = (chrome && chrome.footer) || {};
  const e = ymiatEscapeHtml;
  return (
    '<p>© <strong>' + e(f.brand || 'You-Meet-In-A-Tavern') + '</strong> (YMIAT).</p>' +
    '<p>' + e(f.licenseIntro || '') + ' <a href="https://creativecommons.org/licenses/by/4.0/">' + e(f.licenseName || 'CC BY 4.0') + '</a>.</p>' +
    '<p>' + e(f.srd || '') + '</p>' +
    '<p>' + e(f.disclaimer || '') + '</p>' +
    '<p>' + e(f.trademarks || '') + '</p>'
  );
...
function ymiatSiblingUrl(targetLocale) {
  const hash = window.location.hash || '';
  const search = window.location.search || '';
  const depth = ymiatPathDepth();
  const locale = ymiatDetectLocale();
  const siteParts = ymiatSiteParts();
  const file = siteParts.length ? siteParts[siteParts.length - 1] : 'index.html';
  let afterDirs = siteParts.slice(0, -1);
  if (afterDirs[0] === 'nl') afterDirs = afterDirs.slice(1);

  if (ymiatIsLocalHost()) {
    const after = (afterDirs.concat([file]).join('/')) || 'index.html';
    if (targetLocale === 'nl') {
      if (locale === 'nl') {
        return ymiatRelativePrefix(Math.max(0, depth - 1)) + after + search + hash;
      }
      return ymiatRelativePrefix(depth) + 'nl/' + after + search + hash;
    }
    if (locale === 'nl') {
      return ymiatRelativePrefix(depth) + after + search + hash;
    }
    return ymiatRelativePrefix(depth) + after + search + hash;
  }
  ...
  if (targetLocale === 'nl') {
    return prefix + '/nl/' + after + search + hash;
  }
  return prefix + '/' + after + search + hash;
}

function ymiatBindLangSwitch() {
  document.querySelectorAll('[data-lang-switch]').forEach(function (el) {
    el.addEventListener('click', function (ev) {
      ev.preventDefault();
      const target = el.getAttribute('data-lang-switch');
      if (!target || target === ymiatDetectLocale()) return;
      try { localStorage.setItem('ymiat-lang', target); } catch (err) {}
...
(function(){
  ...
  function updateActiveNav(){
    ...
    const navLinks = document.querySelectorAll('nav a:not([data-lang-switch])');
  ...
  const CS_STORE_KEY = 'ymiat-characters-v1';

  function populateNavCharactersMenu(){
    const menu = document.getElementById('nav-characters-menu');
    if(!menu) return;
    const rootPath = getRootPath();
    const manageHref = rootPath + 'character-sheet.html';
    menu.querySelectorAll('[data-nav-cs-char]').forEach(el => el.remove());

    let characters = [];
    try {
      const raw = localStorage.getItem(CS_STORE_KEY);
      const parsed = raw ? JSON.parse(raw) : null;
      if(parsed && Array.isArray(parsed.characters)) characters = parsed.characters;
    } catch (_) { /* ignore */ }

    characters.forEach(c => {
      if(!c || !c.id) return;
      const a = document.createElement('a');
      a.href = manageHref + '?id=' + encodeURIComponent(c.id);
      a.setAttribute('data-nav-cs-char', c.id);
  ...
  window.ymiatRefreshNavCharacters = populateNavCharactersMenu;
  window.addEventListener('storage', (e) => {
    if(e.key === CS_STORE_KEY || e.key === null) populateNavCharactersMenu();
  });
  window.addEventListener('ymiat-characters-changed', populateNavCharactersMenu);
  ...
  window.ymiatGetAssetsPath = getAssetsPath;
  window.ymiatDetectLocale = ymiatDetectLocale;
})();
